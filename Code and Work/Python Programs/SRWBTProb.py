...
def SRWBTProbsList(mylist,AorB):
    ...
    while keeprunning == True:
        ...
        if len(mylist)==0:
            ...
            SRWBTProbsList(mylist,AorB)

def SRWBTProbsEquation():
...
def SRWBTFinalEquation():
    ...
    final = solve(final,A)
    final = final[0]
    SRWBTMakeValues(final,enoughtowin)


def SRWBTMakeValues(equation,enoughtowin):
    global Values
    valueslist=[]
    ...
    SRWBTMakeExcel(valueslist,enoughtowin)

def SRWBTMakeExcel(valueslist,enoughtowin):
    global Values
    wb = xw.Book()
    ...
    for x in range(0,len(Values)):
        ...
        count = count +1
    sht.cells(2,2).value = "DNE"
    #Finally, we just want to add in a cell the 2k+1 series game for which we are calculating our data
    sht.cells(1,15).value = ("{}-game SR WBT series").format((2*enoughtowin)-1)

enoughtowin = 6

SRWBTProbsList(myAlist,1)
keeprunning = True
SRWBTProbsList(myBlist,0)
SRWBTProbsEquation()
